...
改为允许重新生成，但要同时满足两个相互冲突的需求：

- **泄露时要能立刻止损** → 重新生成必须随时可用
- **不能成为绕过冷却换设备的后门** → 否则冷却机制形同虚设

按**新令牌绑定哪台设备**来区分：

| 情况 | 处理 |
|---|---|
| 重新生成 | **立即执行**。旧令牌 `revoked_at` 置位，其 License `jti` 进吊销名单 |
| 新令牌绑定**原设备**（fingerprint 相同） | **立即放行**。这是止损，没有换设备 |
| 新令牌绑定**不同设备** | **受冷却约束**，冷却期内 `validate` 返回 `REBIND_COOLDOWN` |

这样泄露的令牌能被立刻作废（攻击者立即失效），而想借重新生成来换设备的人仍然被冷却挡住。
...
| 项 | 说明 |
|---|---|
| **group slug 拼写** | 文档按 `sponsor` 编写。Prism 中 slug 创建后不可变，下游全部绑定它，确认后再创建 |
| **冷却时长** | 沿用 24 小时（原代码值）还是 30 天（原注释值）？存于 `site_config` |
...
需要官网会话 + `level >= 1`。已有令牌时重新生成，旧令牌立即作废。

```json
{ "ok": true, "token": "…" }
```

新令牌绑定到原设备不受冷却限制；绑定到不同设备时，`validate` 在冷却期内返回 `REBIND_COOLDOWN`。
